  ...
  return (
    <PixelPanel className="mb-5 animate-rise">
      <PixelHeader
        ...
        right={<span className="text-[9px] text-dv-bg/85 tracking-wider">3 ENTRIES / DAY</span>}
      />
      ...
      <div className="flex items-start justify-between gap-4 mb-4">
        <div>
          ...
          <p className="text-[10px] leading-relaxed text-slate-200/75 mt-1">Cooking helps the raid phase; exploration earns materials, points, and trace chances.</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        {[1, 2, 3].map((entry) => (
          <div key={entry} className="pixel-frame border border-dv-line bg-dv-bg/70 px-2 py-3 text-center">
            ...
            <span className="block text-[9px] text-dv-brassLight mt-1">ENTRY {entry}</span>
            <span className="block text-[8px] text-slate-200/65 mt-1">AVAILABLE</span>
          </div>
        ))}
      </div>
      ...
      <div className="pixel-frame border border-dv-violet/60 bg-dv-panel2 px-3 py-3 flex items-start gap-2">
        ...
        <div>
          <p className="text-[10px] text-dv-brassLight">WYVERN INTEL HIDDEN</p>
          <p className="text-[10px] leading-relaxed text-slate-200/75 mt-1">Fill the exploration progress bar and watch for traces. Wyvern tracking unlocks on Day 3 when a Wyvern is recorded.</p>
        </div>
      </div>

      <p className="text-[8px] text-slate-200/60 mt-3 tracking-wide">EXPLORE • COOK SPECIAL DISHES • BUILD RAID ADVANTAGE</p>
    </PixelPanel>
  );
}
