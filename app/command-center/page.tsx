...
function Board() {
  const [logged, setLogged] = useState<boolean[]>(() => {
    if (typeof window === "undefined") return [true, true, false, false, false, false];
    try { return JSON.parse(localStorage.getItem("dv-command-attacks") ?? "[true,true,false,false,false,false]"); } catch { return [true, true, false, false, false, false]; }
  });
  useEffect(() => { localStorage.setItem("dv-command-attacks", JSON.stringify(logged)); }, [logged]);
  const completed = logged.filter(Boolean).length;

  return <div className="space-y-4">
    <Card className="overflow-hidden">
      <div className="flex items-center justify-between border-b border-dv-line px-4 py-3"><div><SectionLabel>CURRENT SEASON CLOCK</SectionLabel><p className="text-[11px]">CYCLE 18 <span className="text-slate-300/50">/ DAY 3 OF 6</span></p></div><span className="status-chip">WEDNESDAY</span></div>
      <div className="grid grid-cols-7 gap-px bg-dv-line">
        {["MON", "TUE", "WED", "THU", "FRI", "SAT", "SUN"].map((day, index) => <div key={day} className={`bg-dv-panel px-1 py-3 text-center ${index === 2 ? "bg-dv-violet/15" : ""}`}><p className={`text-[9px] ${index < 2 ? "text-dv-emerald" : index === 2 ? "text-dv-brassLight" : "text-slate-300/55"}`}>{day}</p><div className={`mx-auto my-2 h-2 w-2 ${index < 2 ? "bg-dv-emerald" : index === 2 ? "bg-dv-brass" : "border border-dv-line"}`} /><p className="text-[9px] text-slate-300/60">{index < 3 ? (index === 0 ? "Match" : "Explore") : "Raid"}</p></div>)}
      </div>
      <p className="border-t border-dv-line px-4 py-2 text-[9px] text-slate-300/60">◆ Exploration closes in <span className="text-dv-brassLight">18h 22m</span>; raid prep starts Friday.</p>
    </Card>

    <div className="grid gap-4 lg:grid-cols-[1.3fr_.7fr]">
      <Card className="animate-rise">
        <div className="flex items-start justify-between gap-4 border-b border-dv-line px-4 py-4"><div><SectionLabel>YOUR RAID LEDGER</SectionLabel><h2 className="text-lg text-dv-brassLight">Wednesday attack</h2><p className="mt-1 text-xs text-slate-200/65">One clean check-in keeps the guild readout honest.</p></div><div className="text-right"><p className="eyebrow">RESET WINDOW</p><p className="mt-1 font-pixel text-sm text-dv-emerald">18:22:14</p></div></div>
        <div className="px-4 py-4"><div className="mb-3 flex justify-between text-[9px] uppercase tracking-[.1em] text-slate-300/60"><span>Cycle progress</span><span className="text-dv-brassLight">{completed}/6 logged</span></div><div className="grid grid-cols-6 gap-1.5">{logged.map((done, index) => <div key={index} className={`h-2 border ${done ? "border-dv-emerald bg-dv-emerald" : index === 2 ? "border-dv-brass bg-dv-brass/15" : "border-dv-line bg-dv-panel2"}`} />)}</div><button type="button" disabled={logged[2]} onClick={() => setLogged((current) => current.map((value, index) => index === 2 ? true : value))} className={`attack-action mt-5 ${logged[2] ? "is-done" : ""}`}><span className="attack-sigil">◆</span><span className="attack-copy">{logged[2] ? "ATTACK SEALED" : "LOG TODAY'S ATTACK"}<small>{logged[2] ? "RECORDED LOCALLY" : "DAY 3 ATTACK READY"}</small></span></button></div>
      </Card>
      <Card className="relative overflow-hidden p-4"><p className="eyebrow">FIELD NOTE / DAY 3</p><h2 className="mt-1 max-w-[230px] text-xl text-dv-brassLight">Trace hunt is still open.</h2><p className="mt-3 max-w-[290px] text-xs leading-relaxed text-slate-200/65">Use all three exploration entries today. The trace decides which wyvern the raid team can prepare for.</p><p className="mt-5 text-[10px] uppercase text-dv-violet">◆ 3 entries available</p></Card>
    </div>
    <Card className="p-4"><div className="mb-4 flex items-end justify-between"><div><SectionLabel>GUILD READOUT</SectionLabel><h2 className="text-lg text-dv-brassLight">Keep the line moving.</h2></div><span className="status-chip">4 pending</span></div><div className="grid grid-cols-3 gap-2"><div className="item-slot p-3"><p className="eyebrow">TODAY</p><p className="mt-1 text-xl text-dv-emerald">2<span className="text-xs text-slate-300/50">/6</span></p></div><div className="item-slot p-3"><p className="eyebrow">POINTS</p><p className="mt-1 text-xl text-dv-brassLight">21.7k</p></div><div className="item-slot p-3"><p className="eyebrow">CLEAN RUNS</p><p className="mt-1 text-xl text-dv-brassLight">2</p></div></div></Card>
  </div>;
}
