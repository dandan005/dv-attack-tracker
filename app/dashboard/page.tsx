...
export default function DashboardPage() {
  ...
  async function loadAll() {
    ...
    const { data: logs } = await supabase.from("attack_logs").select("member_id, day_number").eq("cycle_start", cycle.cycleStartISO);

    const logsByMember = new Map<string, Set<number>>();
    (logs ?? []).forEach((log: any) => {
      if (!logsByMember.has(log.member_id)) logsByMember.set(log.member_id, new Set());
      logsByMember.get(log.member_id)!.add(log.day_number);
    });
    ...
    const rows: MemberRow[] = (allMembers ?? []).map((member: any) => ({
      ...
      logged: [1, 2, 3, 4, 5, 6].map((day) => logsByMember.get(member.id)?.has(day) ?? false),
    }));
    setMembers(rows);

    const myRow = (allMembers ?? []).find((member: any) => member.auth_user_id === user.id);
    if (myRow) {
      setMyLoggedDays([1, 2, 3, 4, 5, 6].filter((day) => logsByMember.get(myRow.id)?.has(day)));
  ...
  if (loading) {
    return <main className="min-h-screen flex items-center justify-center"><p className="text-[10px] text-dv-brassLight animate-blink">LOADING GUILD DATA...</p></main>;
  }
    ...
    <>
      <header className="sticky top-0 z-50 px-4 py-3 bg-dv-bg/90 backdrop-blur-md border-b border-dv-line">
        <div className="max-w-2xl mx-auto flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <div className="grid place-items-center w-9 h-9 pixel-frame bg-dv-brass text-dv-bg text-[9px] shadow-pixel-sm shrink-0">DV</div>
            <div className="min-w-0">
              <p className="eyebrow truncate">DRAGON VALLEY // LIVE BOARD</p>
              <h1 className="text-[11px] sm:text-xs text-dv-brassLight mt-1">ATTACK LEDGER</h1>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <span className="status-chip hidden sm:inline-flex">D{dayNumber} ACTIVE</span>
            {me && <div className="pixel-frame item-slot px-2 py-1 flex items-center gap-2"><img src={me.avatar_url ?? "/icons/icon-192.png"} alt="" className="w-6 h-6 pixel-frame border border-dv-line" /><span className="text-[8px] text-dv-brassLight max-w-[70px] truncate">{me.username}</span></div>}
            <button type="button" aria-label="Open settings" onClick={() => router.push("/settings")} className="pixel-frame item-slot border border-dv-line px-3 py-2 text-[10px] shadow-pixel-sm hover:border-dv-violet">⚙️</button>
          </div>
        </div>
      </header>
      ...
      <main className="min-h-screen px-4 pt-6 pb-24 max-w-2xl mx-auto">
        <div className="flex items-end justify-between gap-4 mb-5">
          <div>
            <p className="eyebrow">CYCLE STARTED {cycleStartISO}</p>
            <h2 className="text-xl sm:text-2xl text-dv-brassLight mt-2">KEEP THE GUILD<br className="sm:hidden" /> ON PACE.</h2>
          </div>
          <div className="text-right shrink-0"><p className="text-[8px] text-slate-300/50">CURRENT DAY</p><p className="text-2xl text-dv-emerald">D{dayNumber}</p></div>
        </div>
