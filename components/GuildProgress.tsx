...
export function GuildProgress({
  ...
  return (
    <PixelPanel className="mb-5 animate-rise">
      <PixelHeader icon="⚔️" title="GUILD PROGRESS" right={<span className="text-[8px] bg-dv-bg/25 text-dv-bg px-2 py-1 pixel-frame">{total} MEMBERS</span>} />
      ...
      <div className="pixel-frame border border-dv-line bg-dv-bg/70 p-3 mb-4">
        ...
        <div className="grid grid-cols-3 gap-2 mt-4 text-[8px]">
        ...
        {you && <p className={missCount(you) > 0 ? "text-[8px] text-dv-ember mt-3" : "text-[8px] text-dv-emerald mt-3"}>{missCount(you) > 0 ? "YOUR RUN HAS " + missCount(you) + " MISS" + (missCount(you) === 1 ? "" : "ES") : "YOUR RUN IS CLEAN SO FAR"}{you.score > 0 ? " · " + you.score.toLocaleString() + " POINTS" : ""}</p>}
      </div>
            ...
            {members.map((member) => (
              <div key={member.discord_id} className="grid grid-cols-[1fr_repeat(6,minmax(0,1fr))] gap-1 items-center" title={member.score > 0 ? member.username + " · " + member.score.toLocaleString() + " raid points" : member.username}>
                <div className="flex items-center gap-1 min-w-0 pr-1">
                  ...
                  <div className="min-w-0"><span className="block text-[8px] truncate text-slate-200/80">{member.username}</span><span className="block text-[7px] truncate text-slate-300/35">{member.score > 0 ? member.score.toLocaleString() + " pts" : "no score"}</span></div>
                </div>
      ...
      <button onClick={onPingMissing} disabled={missingToday === 0 || pinging} className="mt-4 w-full pixel-frame bg-dv-ember disabled:bg-dv-panel2 disabled:text-slate-300/35 disabled:border-dv-line text-dv-bg text-[9px] py-3 shadow-pixel-sm active:translate-y-[2px] hover:bg-dv-brassLight transition-colors">
